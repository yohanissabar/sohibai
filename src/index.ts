    ...
    if (url.pathname === "/api/chat" && request.method === "POST") {
      ...
      if (affectionLevel >= 75) {
        ...
        relationshipDynamic =
          ...
          `Tunjukkan perhatian, empati, rasa percaya dan kehangatan secara alami.`;

      } else if (affectionLevel >= 40) {

        relationshipDynamic =
          `Kamu adalah sahabat dekat yang suportif dan selalu siap membantu ${userName}.`;

      } else {

        relationshipDynamic =
          `Kamu adalah teman yang sopan, netral dan membantu ${userName}.`;
...
- Jangan berlebihan memanggil sayang.
- Jawaban maksimal dua paragraf.
- Fokus menjawab isi pertanyaan user.
- Gunakan memori jika memang relevan.
